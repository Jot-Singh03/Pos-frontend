...
const POS: React.FC = () => {
  ...
  const removeFromCart = (itemId: string) => {
    setCart(prevCart => {
      const item = prevCart.find(cartItem => cartItem.item._id === itemId);
      if (item) {
        toast.info(`${item.item.name} removed from cart`);
      }
  ...
  const handleCheckout = async () => {
    ...
    try {
      const orderData = {
        ...
        totalAmount: cart.reduce((sum, cartItem) => sum + cartItem.item.price * cartItem.quantity, 0),
            ...
            <div style={{
              ...
              {items.map(item => (
                <div key={item._id} style={{
                  ...
                  />
                  <div style={{ fontWeight: 600, fontSize: '1.1rem', marginBottom: 4 }}>{item.name}</div>
                  <div style={{ color: theme.colors.gray[700], fontSize: '0.95rem', marginBottom: 4 }}>{item.description}</div>
                  <div style={{ color: theme.colors.primary, fontWeight: 700, fontSize: '1rem', marginBottom: theme.spacing.sm }}>
            ...
            <div style={{ 
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <label style={{ fontWeight: 600, fontSize: '1rem', marginBottom: 4 }} htmlFor="phoneNumber">
                    Phone Number
                  </label>
                  <input
                    id="phoneNumber"
                ...
                <div style={{ textAlign: 'center', color: '#888', fontWeight: 500 }}>or</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <label style={{ fontWeight: 600, fontSize: '1rem', marginBottom: 4 }} htmlFor="tableToken">
                    Table Token
                  </label>
                  <select
                    id="tableToken"
                    value={tableToken}
                    onChange={e => setTableToken(e.target.value ? Number(e.target.value) : '')}
                    style={{
                      padding: '0.75rem 1rem',
                      border: '1px solid #d1d5db',
                      borderRadius: 8,
